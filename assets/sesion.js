(async () => {
  ...
  let scrollT; window.addEventListener("scroll", () => {
    if (!S.teacher || !isMobile() || !S.stageFrame || S.stageFrame.dataset.auto !== "1") return;
    clearTimeout(scrollT); scrollT = setTimeout(() => { const top = S.stageFrame.getBoundingClientRect().top + window.scrollY, hgt = S.stageFrame.offsetHeight; const r = Math.min(1, Math.max(0, (window.scrollY - top + 70) / Math.max(1, hgt))); const sec = "scroll:" + r.toFixed(3); if (sec !== S.lastSection) { S.lastSection = sec; sb.from("sessions").update({ projected_state: { section: sec, label: "Desplazamiento", at: new Date().toISOString() } }).eq("id", sessionId).then(() => {}); } }, 300);
    // vídeo en miniatura al bajar (alumno)
  }, { passive: true });
  window.addEventListener("scroll", () => { if (S.teacher || !isMobile()) return; const mini = window.scrollY > 220; if (mini !== document.body.classList.contains("auto-mini")) document.body.classList.toggle("auto-mini", mini); }, { passive: true });
  // cabecera que se esconde al bajar
  let lastY = 0; window.addEventListener("scroll", () => { if (S.teacher || !isMobile()) return; const y = window.scrollY; document.body.classList.toggle("top-hidden", y > lastY && y > 80); lastY = y; }, { passive: true });
  ...
  window.addEventListener("message", async e => {
    ...
    if (d.type === "height" && isMobile() && d.h) { S.stageFrame.style.height = Math.max(300, d.h + 20) + "px"; S.stageFrame.dataset.auto = "1"; return; }
    if (d.type === "scrollto" && isMobile() && typeof d.y === "number") { const top = S.stageFrame.getBoundingClientRect().top + window.scrollY; window.scrollTo({ top: top + d.y - 70, behavior: "smooth" }); return; }
