(async () => {
  ...
  const S = { e: null, c: null, items: {}, view: "home", idx: 0, frame: null, cache: {}, questions: [], next: null };
  const eid = qs("e");
  ...
  async function load() {
    ...
    if (ids.length) {
      const { data: li } = await sb.from("library_items").select("*").in("id", ids);
      (li || []).forEach(i => S.items[i.id] = i);
    }
    if (S.c.type === "live" && S.c.group_id) {
      const { data: ss } = await sb.from("sessions").select("id, title, starts_at, status").eq("group_id", S.c.group_id).neq("status", "closed").gte("starts_at", new Date(Date.now() - 3600e3).toISOString()).order("starts_at").limit(1);
      S.next = (ss || [])[0] || null;
    }
    const { data: qs2 } = await sb.from("course_questions").select("*").eq("course_id", S.c.id).eq("user_id", me.user.id).order("created_at", { ascending: false });
    S.questions = qs2 || [];
    return true;
  }

  const done = () => new Set((S.e.progress?.done || []).map(Number));
  const total = () => S.c.lessons.length;
  const pct = () => total() ? Math.round(done().size / total() * 100) : 0;
  const finished = () => !!S.e.completed_at;
  const deadlinePassed = () => !finished() && S.e.ends_at && new Date(S.e.ends_at).getTime() < Date.now();
  const reviewPassed = () => finished() && S.e.review_until && new Date(S.e.review_until).getTime() < Date.now();
  const canOpenLessons = () => finished() ? !reviewPassed() : !deadlinePassed();
  const daysLeft = () => S.e.ends_at ? Math.ceil((new Date(S.e.ends_at) - Date.now()) / 86400e3) : null;
  const currentIdx = () => { const d = done(); const i = S.c.lessons.findIndex((_, k) => !d.has(k)); return i < 0 ? Math.max(total() - 1, 0) : i; };
  ...
  function renderTop() {
    ...
    renderShell(me, "");
    $("back")?.addEventListener("click", () => { S.view = "home"; render(); });
  }
  ...
  function render() {
    ...
    const d = daysLeft(), cur = currentIdx(), fin = finished(), late = deadlinePassed(), reviewEnd = reviewPassed();

    let statusCard = "";
    if (fin) {
      statusCard = `<div class="cert"><h3>¡Curso terminado!</h3><p>${esc(S.c.title)} · ${total()} de ${total()} lecciones · ${fmtDate(S.e.completed_at, { day: "numeric", month: "long", year: "numeric" })}</p><a class="button gold small" href="certificado.html?e=${S.e.id}" target="_blank">Ver mi certificado</a></div>` +
        (reviewEnd
          ? `<div class="k-card"><b>El periodo de repaso ha terminado.</b><p class="meta" style="margin:4px 0 0">Tu certificado seguirá disponible siempre en Mi campus.</p></div>`
          : `<div class="k-card"><b>Puedes repasar hasta el ${fmtDate(S.e.review_until, { day: "numeric", month: "long", year: "numeric" })}</b><p class="meta" style="margin:4px 0 0">Después se cerrarán las lecciones, pero conservarás tu certificado.</p></div>`);
    } else if (late) {
      statusCard = `<div class="k-card"><span class="eyebrow">Tu curso</span><h2>${esc(S.c.title)}</h2><div class="prog"><i style="width:${pct()}%"></i></div><p class="meta" style="margin:0">${done().size} de ${total()} lecciones · ${pct()} %</p><div class="notice" style="margin-top:12px"><b>El plazo para completar este curso terminó el ${fmtDate(S.e.ends_at, { day: "numeric", month: "long", year: "numeric" })}.</b><br>Tu progreso se conserva. Si necesitas repetirlo, pide a coordinación que reinicie tu matrícula.</div></div>`;
    } else {
      statusCard = `<div class="k-card"><span class="eyebrow">Tu curso</span><h2>${esc(S.c.title)}</h2><div class="prog"><i style="width:${pct()}%"></i></div><p class="meta" style="margin:0">Lección ${Math.min(cur + 1, total())} de ${total()} · ${pct()} %${d !== null ? ` · <b>${d > 0 ? "te quedan " + (d >= 14 ? Math.round(d / 7) + " semanas" : d + " días") : "último día"}</b>` : ""}</p>
    ...
    main.innerHTML = `
      ${statusCard}
      ${S.next && !fin && !late ? `<div class="k-card next-live"><b>${S.next.status === "live" ? "Clase en directo ahora" : "Próxima clase en directo"}</b><p class="meta" style="margin:2px 0 8px">${esc(S.next.title)} · ${fmtDate(S.next.starts_at)}</p><a class="button small ${S.next.status === "live" ? "gold" : "secondary"}" href="sesion.html?id=${S.next.id}">${S.next.status === "live" ? "Entrar" : "Ver la clase"}</a></div>` : ""}
      ${total() ? `<div class="k-card"><h3>Lecciones</h3><ul class="lessons">${S.c.lessons.map((l, i) => { const isDone = done().has(i), isCur = i === cur && !fin && !late, lock = !canOpenLessons() || (!isDone && i > cur); return `<li class="${isDone ? "done" : isCur ? "cur" : lock ? "lock" : ""}" data-l="${i}"><span class="ic">${isDone ? "✓" : isCur ? "●" : lock ? "🔒" : "▶"}</span>${i + 1} · ${esc(l.title || S.items[l.library_item_id]?.title || "Lección")}</li>`; }).join("")}</ul></div>` : ""}
      ...
      ${!fin && !late ? `<div class="cert"><h3>Al terminar</h3><p>Recibirás tu certificado con tu nombre y podrás elegir el siguiente curso.</p><a class="button gold small" href="index.html#cursos">Ver otros cursos</a></div>` : fin ? `<div class="k-card"><h3>Siguiente paso</h3><a class="button" href="index.html#cursos">Ver otros cursos</a></div>` : `<div class="k-card"><h3>¿Quieres repetirlo?</h3><p class="meta">Coordinación puede reiniciar tu matrícula conservando el historial administrativo.</p></div>`}`;

    main.querySelector("#continue")?.addEventListener("click", () => openLesson(cur));
    main.querySelectorAll("[data-l]").forEach(li => li.addEventListener("click", () => {
      const i = Number(li.dataset.l);
      if (!canOpenLessons()) { toast(fin ? "El periodo de repaso ha terminado." : "El plazo para completar el curso ha terminado."); return; }
      if (li.classList.contains("lock") && !done().has(i)) { toast("Termina antes la lección " + (cur + 1)); return; }
      openLesson(i);
    }));
  ...
  async function openLesson(i) {
    if (!canOpenLessons()) { toast(finished() ? "El periodo de repaso ha terminado." : "El plazo para completar el curso ha terminado."); return; }
    S.idx = i; S.view = "lesson"; render();
  }

  async function renderLesson(main) {
    if (!canOpenLessons()) { S.view = "home"; render(); return; }
    ...
    if (!item) { stage.innerHTML = `<p class="notice">Esta lección no está disponible o el acceso ha finalizado.</p>`; return; }
  ...
  window.addEventListener("message", e => {
    ...
    if (d.type === "answers") {
      ...
      saveT = setTimeout(async () => {
        if (!canOpenLessons() || finished()) return;
        const { data: p, error } = await sb.rpc("course_save_answers", { p_enrollment: S.e.id, p_lesson: S.idx, p_answers: d.answers || {} });
        if (error) { if (/PLAZO_TERMINADO/.test(error.message || "")) toast("El plazo para completar el curso ha terminado."); return; }
        S.e.progress = p || S.e.progress;
        const el = $("saved"); if (el) el.textContent = "Respuestas guardadas ✓";
        if (d.answers && d.answers["Puntuación"] && !done().has(S.idx)) markDone(S.idx, true);
      }, 1200);
    }
  });

  async function markDone(i, silent) {
    if (finished()) { if (!silent && i + 1 < total()) openLesson(i + 1); return; }
    const { data, error } = await sb.rpc("course_mark_lesson_done", { p_enrollment: S.e.id, p_lesson: i });
    if (error) {
      const m = String(error.message || "");
      if (/PLAZO_TERMINADO/.test(m)) toast("El plazo para completar el curso ha terminado.");
      else if (/LECCION_BLOQUEADA/.test(m)) toast("Completa primero la lección anterior.");
      else toast("No se pudo guardar: " + m);
      return;
    }
    if (data?.progress) S.e.progress = data.progress;
    if (data?.completed_at) { S.e.completed_at = data.completed_at; S.e.review_until = data.review_until; S.view = "home"; render(); toast("¡Enhorabuena, curso terminado!"); return; }
    if (!silent) { toast("Lección " + (i + 1) + " hecha"); if (i + 1 < total()) openLesson(i + 1); else { S.view = "home"; render(); } }
    else { const el = $("saved"); if (el) el.textContent = "Lección hecha ✓"; }
  }
